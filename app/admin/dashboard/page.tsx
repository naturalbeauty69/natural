...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    { label: "Pending Appointments", value: stats.pendingAppointments, icon: Clock },
    { label: "Running Batches", value: stats.runningBatches, icon: GraduationCap },
    { label: "Upcoming Classes", value: stats.upcomingBatches, icon: GraduationCap },
    { label: "Total Students", value: stats.totalStudents, icon: Users },
    { label: "Total Staff", value: stats.totalStaff, icon: Briefcase },
    { label: "New Enrollments (30d)", value: stats.newEnrollments30d, icon: UserCheck },
    { label: "Customer Reviews", value: stats.reviews, icon: Star },
  ];

  return (
    <div>
      <h1 className="font-display text-2xl text-emerald-900 dark:text-cream">Dashboard</h1>
      ...
      <div className="mt-8 rounded-xl2 border border-dashed border-gold-500/40 bg-gold-100/30 p-6 dark:bg-emerald-900/40">
        <p className="eyebrow flex items-center gap-2 text-gold-700">
          <TrendingUp className="h-4 w-4" /> Charts (Student Growth, Admissions, Popular Courses/Services, Appointment Trends)
        </p>
        <p className="mt-2 text-sm text-ink-soft dark:text-cream/70">
          These become meaningful once there&apos;s real usage history to chart — the stat cards above are already
          live from your database. Charting library (Recharts) is installed and ready; wiring the actual
          trend charts is a quick follow-up once a few weeks of real appointment/enrollment data exist.
        </p>
      </div>
    </div>
  );
